...
def load_game_data() -> dict:
    ...
    game_data['families'] = {f['id']: f for f in battle_config.get('families', [])}
    game_data['family_effects'] = {fe['id']: fe for fe in battle_config.get('familyEffects', [])}
    ...
    key_groups = [
        ...
        "statusEffectTagsWithExtraDescription",
        "ownedStatusEffectTagsWithExtraDescription",
...
def load_passive_master(path: Path) -> dict:
    """Loads optional passive lang/icon mapping maintained outside game JSON."""
    if not path.exists():
        print(f" -> Passive master not found: {path.name}")
        return {}
    try:
        ...
        print(f" -> Loaded {len(master)} passive master rows.")
        return master
    except Exception as e:
...
def load_source_texts(path: Path) -> dict:
    """Loads optional source-text hints generated by Hero Text Scraper."""
    if not path.exists():
        print(f" -> Source text hint dir not found: {path}")
        return {}
    source_texts = {}
    for file_path in path.glob("*.json"):
        try:
            ...
            print(f" -> Warning: Could not load source text {file_path.name}. Error: {e}")
            continue
        hero_id = data.get("hero_id") or file_path.stem
        source_texts[hero_id] = data
    print(f" -> Loaded {len(source_texts)} source text hint files.")
    return source_texts
...
def load_hero_stats_from_csv(base_dir: Path, pattern: str) -> dict:
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Keep all columns. Some calculations need costume-specific columns
            # such as "CB4 Max Attack", and the exact costume number varies.
            entry = row.to_dict()
